			...
			if(true){
				...
				function computePixel(x, y, nodes){
					...
					pixel.heat = _(nodes).reduce(function(sum, node){
						...
						var distance_ego_to_pixel = Math.sqrt(Math.pow(egoX - x,2)+Math.pow(egoY - y,2)) / (Math.min(data.canvas.w, data.canvas.h) - 2 * data.margin);
						
						var nodeHeat;
						if(data.weighted){
							nodeHeat = Math.sqrt(node.size) * Math.exp(- d * node.t_half / distance_ego_to_pixel) / distance_ego_to_pixel;
						} else {
							nodeHeat = Math.exp(- node.t_half * d / distance_ego_to_pixel) / distance_ego_to_pixel;
						}
						...
						_(pixMap).each(function(col,x){
							_(col).each(function(pixel,y){
								if(pixel.computed){
									...
									var rgb = hslToRgb(data.color.min + normalized_heat*(data.color.max-data.color.min),1,0.5);
									pixel.r = rgb[0];
									pixel.g = rgb[1];
									pixel.b = rgb[2];
									pixel.a = 255;
