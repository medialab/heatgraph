			...
			if(true){
				...
				function computePixel(x, y, nodes){
					...
					_(nodes).each(function(node){
						...
						if(nodeHeat > closeNodeHeat){
							closeNode = node;
							closeNodeHeat = nodeHeat;
							
							pixel.heat = closeNodeHeat;
							if(pixel.heat>maxHeat)
								maxHeat = pixel.heat;
						}
					});
					
					pixel.color = closeNode.color;
						...
						_(pixMap).each(function(col,x){
							_(col).each(function(pixel,y){
								if(pixel.computed){
									...
									var hsl = rgbToHsl(pixel.color.r * 255, pixel.color.g * 255, pixel.color.b * 255);
									var rgb = hslToRgb(hsl[0], hsl[1], data.lightnessRatio * normalized_heat);
